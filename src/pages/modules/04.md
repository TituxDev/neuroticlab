...
Incluso el sesgo (bias) se puede tratar como un peso con una entrada constante igual a 1.
...
A simple vista, salta el problema de intentar usar derivadas con esta función: no hay derivada que calcular. La pendiente es cero a lo largo de toda la función, excepto en el punto exacto donde cambia de 0 a 1, y ahí ni siquiera está bien definida.
...
Sí, tiene un nombre raro, pero tranquilo, no es nada del otro mundo. Se llama así porque tiene forma de “S” —así de simple fue la inspiración de los matemáticos.
...
/*=========CALCULAR==========*/
/*=========ENTRENAR==========*/
int entrenar( float entradas[4][2], float resultados[4], float tasa_aprendizaje, float error_permitido, int max_intentos, struct neurona *N  ){
    float error;
    float delta;
    float delta_oculta;
    float error_total;
...
- Se agregaron nuevas variables internas: delta, delta_oculta y error_total, y se cambió el tipo de la variable error a float para que sea compatible con los nuevos cálculos decimales.
...
int main(){
    ...
    /* ENTRENAMIENTO */
    printf( "\nEntrenamiento Intentos: %i\n" , entrenar( tabla , resultado , 0.1, 0.0, 1000, N) );
    /* CALCULAR E IMPRIMIR */
        printf( "\n\n| A | B | S |\n" );
        for( int i= 0 ; i < 4 ; i++){
            for( int j= 0 ;  j < 2 ; j++)
...
Algo en lo que quiero que prestes atención antes de terminar este módulo es al parámetro de entrenamiento llamado error_permitido. Este valor representa un margen de error aceptable, y ha estado presente en todas las redes neuronales que has entrenado hasta ahora.

Esto no es un detalle menor. Toda inteligencia artificial con la que llegues a interactuar —ya sea un asistente virtual, un traductor automático o un sistema de recomendación— fue entrenada con un criterio similar. Eso significa que siempre hay un pequeño rango en el que la IA puede equivocarse… y ese margen fue considerado “suficientemente bueno”.
